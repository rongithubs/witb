    ...
    <div className="bg-green-50 border-l-4 border-green-400 p-4 mb-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            ...
            <div className="mt-1 text-sm text-green-700">
              ...
              {winnerData.date && (
                <span className="ml-2 text-green-600">• {winnerData.date}</span>
              )}
            </div>
          </div>
        </div>
        ...
        {winnerPlayer && winnerPlayer.witb_items.length > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-green-700">See what's in their bag</span>
            <button
      ...
      {isExpanded && winnerPlayer && (
        <div className="mt-4 pt-4 border-t border-green-200">
          <h4 className="text-sm font-medium text-green-800 mb-3">What's In The Bag</h4>
          <div className="space-y-2">
            {winnerPlayer.witb_items.length === 0 ? (
              <p className="text-green-600 italic text-xs">No equipment data available</p>
            ) : (
              winnerPlayer.witb_items.map((club, index) => (
                <div key={index} className="border border-green-200 p-3 rounded bg-green-25">
                  <div className="font-medium text-green-800 text-sm">{club.category}</div>
                  <div className="text-green-700 text-sm">{club.brand} {club.model}</div>
                  {club.loft && <div className="text-green-600 text-xs">Loft: {club.loft}</div>}
                  {club.shaft && <div className="text-green-600 text-xs">Shaft: {club.shaft}</div>}
                </div>
              ))
            )}
          </div>
