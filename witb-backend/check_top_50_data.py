...
async def show_top_50_data():
    ...
    async with async_session() as session:
        ...
        print(f'   • Players scraped: {scraped_players}/50 ({scraped_players/50*100:.1f}% success rate)')
        print(f'   • Total equipment items: {total_items}')
        print(f'   • Average items per player: {total_items/scraped_players:.1f}' if scraped_players > 0 else '   • Average items per player: 0')
        ...
        driver_result = await session.execute(text('''
